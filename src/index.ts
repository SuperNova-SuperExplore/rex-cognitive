...
const FALLACY_PATTERNS: Array<{ name: string; category: string; pattern: RegExp; description: string }> = [
  ...
  { name: "FALSE_DICHOTOMY", category: "structural", pattern: /\b(either\s+(?!there\s+(exist|is|are|was))(?!.{0,30}\bor\s+(it\s+)?(does|doesn't|doesn't|is|isn't|isn't)).{3,30}\s+or\b|only two (options|choices|ways)|no (middle ground|other option|alternative))/i, description: "Presenting only two options when more exist" },
  ...
  { name: "CIRCULAR_REASONING", category: "structural", pattern: /\b(true because.{0,30}true|right because.{0,30}right|is\s+(\w+)\s+because\s+it\s+is\s+\2)/i, description: "Using conclusion as premise" },
